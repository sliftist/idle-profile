...
export function formatTime(milliseconds: number): string {
    ...
    if(milliseconds < 1/1000) {
        ...
        return formatMinDigits(milliseconds / 1000 / 60, 3) + "m";
    } else if(milliseconds < 1000 * 60 * 60 * 24) {
        return formatMinDigits(milliseconds / 1000 / 60 / 60, 3) + "h";
    } else if(milliseconds < 1000 * 60 * 60 * 24 * 7) {
        return formatMinDigits(milliseconds / 1000 / 60 / 60 / 24, 3) + "d";
    } else {
        let days = Math.round(milliseconds / 1000 / 60 / 60 / 24 / 7);
        return `${days}w`;
    }
}
